        ...
        <div class="row">
            <div class="col-sm-12">
              <div class="card card-table">
                <div class="card-header">All Users Information
                </div>
                @if (session('success'))
                    ...
                    <table class="table table-striped table-hover table-fw-widget" id="table4">
                        ...
                        <tbody>
                            ...
                            <tr class="odd gradeX">
                                ...
                                <td>
                                    @if ($user->image != "")
                                        <img style="width: 50px;border-radius:50%;object-fit:cover" src="{{ asset('themes/admin/users') }}/{{ $user->image }}" alt="Profile Image">
                                    @else
                                        <span>Empty</span>
                                    @endif
                                </td>
                                <td class="center">{{ (($user->name != "")?substr($user->name,0,20).'...':'Empty') }}</td>
                                <td class="center">{{ (($user->email != "")?substr($user->email,0,30).'...':'Empty') }}</td>
                                <td class="center">{{ \Carbon\Carbon::parse($user->created_at)->diffForHumans() }}</td>
                                <td class="center">
                                  @if ($user->status == 1)
                                    <a href="{{ route('admin.user.status.change',encrypt($user->id)) }}" class="btn btn-warning">Block</a>
                                  @else
                                    <a href="{{ route('admin.user.status.change',encrypt($user->id)) }}" class="btn btn-success">Active</a>
                                  @endif
                                    <a href="javascript:void(0)" class="btn btn-danger userDeleteBtn" onclick="event.preventDefault(); if(confirm('Are You Sure?') === true) {
                                      document.getElementById('user-delete-form<?=($key)?>').submit();
                                    }
                                      ">Delete</a>    
                                </td>
                                <form id="user-delete-form<?=($key)?>" action="{{ route('admin.user.delete',encrypt($user->id)) }}" method="POST" class="d-none">
                                  @csrf
                                </form>
                              </tr>
                            @endforeach

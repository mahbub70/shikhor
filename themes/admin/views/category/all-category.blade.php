            ...
            <div class="col-sm-12">
              <div class="card card-table">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('admin.category.add-form') }}" class="btn btn-primary">Add Category</a>
                    ...
                    <table class="table table-striped table-hover table-fw-widget" id="table4">
                        ...
                        <tbody>
                            ...
                            <tr class="odd gradeX">
                                ...
                                <td>
                                    @if ($item->image != "")
                                        <img style="width: 50px;height:50px;border-radius:50%;object-fit:cover" src="{{ asset('themes/admin/category') }}/{{ $item->image }}" alt="Category Image">
                                    @else
                                        <span>Empty</span>
                                    @endif
                                </td>
                                ...
                                <td class="center">
                                    <a href="{{ route('admin.category.edit-form',encrypt($item->id)) }}" class="btn btn-primary">Edit</a>    
                                    <a href="javacript:void(0)" class="btn btn-danger categoryDeleteBtn" onclick="event.preventDefault(); if(confirm('Are You Sure?') === true) {
                                      document.getElementById('category-delete-form<?=($key)?>').submit();
                                    }
                                      ">Delete</a>    
                                </td>
                                <form id="category-delete-form<?=($key)?>" action="{{ route('admin.category.destroy',encrypt($item->id)) }}" method="POST" class="d-none">
                                  @csrf
                                </form>
                              </tr>
                            @endforeach
